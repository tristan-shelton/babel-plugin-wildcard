'use strict';Object.defineProperty(exports,'__esModule',{value:true});exports.default=function(babel){var t=babel.types;return{visitor:{ImportDeclaration:function ImportDeclaration(path,state){var node=path.node,dec=void 0;var src=path.node.source.value;var addWildcard=false,// True if should perform transform
...
if(isExplicitWildcard){path.node.source.value=path.node.source.value.substring(0,src.length-2);src=path.node.source.value}for(var i=0;i<node.specifiers.length;i++){dec=node.specifiers[i];if(t.isImportNamespaceSpecifier(dec)){addWildcard=true;wildcardName=node.specifiers[i].local.name;node.specifiers.splice(i,1)}// This handles { A, B, C } from 'C/*'
...
if(filterNames.length===0){var obj=t.variableDeclaration('const',[t.variableDeclarator(t.identifier(wildcardName),t.objectExpression([]))]);path.insertBefore(obj)}// Get current filename so we can try to determine the folder
...
if(filterNames.length>0){console.log(fancyName,name,id);var _importDeclaration=t.importDeclaration([t.importDefaultSpecifier(t.identifier(fancyName))],t.stringLiteral(name));path.insertAfter(_importDeclaration);continue}// Generate temp. import declaration
